...
extern	nbnxn_sci_t *		mic_nbl_sci;
extern	nbnxn_cj4_t *		mic_nbl_cj4;
extern	nbnxn_excl_t *		mic_nbl_excl;
extern	nbnxn_pairlist_t  *mic_nbl;
extern	int	*	mic_nbat_type;
extern	nbnxn_atomdata_t  *mic_nbat;
extern	interaction_const_t  *mic_iconst;
extern	rvec               *mic_shift_vec;
extern	real  *                     mic_f;
extern	real  *				mic_t_f;
extern	real  *				mic_t_fshift;
extern	real  *				cpu_t_f;
extern	real  *				cpu_t_fshift;
extern	real  *                     mic_fshift;
extern	real  *                     mic_Vc;
extern	real  *                     mic_Vvdw;
extern	real         				*mic_nbat_x;
extern	real         				*mic_ic_tabq_coul_F;
extern	real       *        mic_nbat_nbfp;
extern	int       *         mic_shift;
extern	int 	SCI_ALLOC;
extern	real	*sig_mic_x;
extern	real	*sig_mic_type;
extern	real	*sig_mic_sci;
extern	real  *                     mic_fshift1;
extern	real  *                     mic_Vc1;
extern	real  *                     mic_Vvdw1;
extern	real  *                     mic_f1;
extern	real  *                     mic_fshift2;
extern	real  *                     mic_Vc2;
extern	real  *                     mic_Vvdw2;
extern	real  *                     mic_f2;
extern	real  *                     cpu_fshift;
extern	real  *                     cpu_Vc;
extern	real  *                     cpu_Vvdw;
extern	real  *                     cpu_f;
extern  int  						mic_nthreads;
extern  int							cpu_nthreads;

//nalloc
extern  real  *                     local_mic_f;
extern  real  *                     non_local_mic_f;
extern  real  *				local_mic_t_f;
extern  real  *				local_mic_t_fshift;
extern  real  *				local_cpu_t_f;
extern  real  *				local_cpu_t_fshift;
extern  real  *				non_local_mic_t_f;
extern  real  *				non_local_mic_t_fshift;
extern  real  *				non_local_cpu_t_f;
extern  real  *				non_local_cpu_t_fshift;
extern  real         				*local_mic_nbat_x;
extern  real         				*non_local_mic_nbat_x;
extern  int       *         local_mic_nbat_type;
extern  int       *         non_local_mic_nbat_type;
extern  int 	local_mic_nalloc ;
extern  int 	non_local_mic_nalloc ;
                         ...
                         gmx_wallcycle_t wcycle)
{
	...
	if(ilocality == eintLocal){
		printf("local cal\n");
		mic_f = local_mic_f;
		mic_f1 = mic_f + mic_nbat->nalloc*mic_nbat->fstride;
		mic_f2 = mic_f + mic_nbat->nalloc*mic_nbat->fstride * 2;
		cpu_f = mic_f + mic_nbat->nalloc*mic_nbat->fstride * 3;
		mic_t_f = local_mic_t_f;
		cpu_t_f = local_cpu_t_f;

		if(local_mic_nalloc == 0){
			mic_nbat_x = nbvg->nbat->x;
			sig_mic_x = mic_nbat_x;
			#pragma offload target(mic:0)nocopy(mic_nbat),nocopy(mic_ic_tabq_coul_F),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(1) free_if(0)),in(mic_iconst:length(1)alloc_if(1) free_if(0))//signal(sig_mic_x)
			{
				mic_nbat->x = mic_nbat_x;
				mic_iconst->tabq_coul_F = mic_ic_tabq_coul_F;
			}
			#pragma offload target(mic:1)nocopy(mic_nbat),nocopy(mic_ic_tabq_coul_F),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(1) free_if(0)),in(mic_iconst:length(1)alloc_if(1) free_if(0))//signal(sig_mic_x)
			{
				mic_nbat->x = mic_nbat_x;
				mic_iconst->tabq_coul_F = mic_ic_tabq_coul_F;
			}
			#pragma offload target(mic:2)nocopy(mic_nbat),nocopy(mic_ic_tabq_coul_F),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(1) free_if(0)),in(mic_iconst:length(1)alloc_if(1) free_if(0))//signal(sig_mic_x)
			{
				mic_nbat->x = mic_nbat_x;
				mic_iconst->tabq_coul_F = mic_ic_tabq_coul_F;
			}
			local_mic_nalloc = mic_nbat->nalloc;
		}
		else if(local_mic_nalloc != mic_nbat->nalloc){
			mic_nbat_x = nbvg->nbat->x;
			sig_mic_x = mic_nbat_x;
			// #pragma offload target(mic:0)nocopy(mic_nbat_x:length(local_mic_nalloc*mic_nbat->xstride)alloc_if(0) free_if(1))
			// {}
			// #pragma offload target(mic:1)nocopy(mic_nbat_x:length(local_mic_nalloc*mic_nbat->xstride)alloc_if(0) free_if(1))
			// {}
			// #pragma offload target(mic:2)nocopy(mic_nbat_x:length(local_mic_nalloc*mic_nbat->xstride)alloc_if(0) free_if(1))
			// {}
			
			#pragma offload target(mic:0)nocopy(mic_nbat),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(1) free_if(0))
			{
				mic_nbat->x = mic_nbat_x;
			}
			#pragma offload target(mic:1)nocopy(mic_nbat),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(1) free_if(0))
			{
				mic_nbat->x = mic_nbat_x;
			}
			#pragma offload target(mic:2)nocopy(mic_nbat),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(1) free_if(0))
			{
				mic_nbat->x = mic_nbat_x;
			}
			local_mic_nalloc = mic_nbat->nalloc;
			...
			mic_nbat_x = nbvg->nbat->x;
			sig_mic_x = mic_nbat_x;
			#pragma offload target(mic:0)nocopy(mic_nbat),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(0) free_if(0))
			{
				mic_nbat->x = mic_nbat_x;
			}
			#pragma offload target(mic:1)nocopy(mic_nbat),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(0) free_if(0))
			{
				mic_nbat->x = mic_nbat_x;
			}
			#pragma offload target(mic:2)nocopy(mic_nbat),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(0) free_if(0))
			{
				mic_nbat->x = mic_nbat_x;
			}
		}
	}
	else if(ilocality == eintNonlocal){
		printf("Nonlocal cal\n");
		mic_f = non_local_mic_f;
		mic_f1 = mic_f + mic_nbat->nalloc*mic_nbat->fstride;
		mic_f2 = mic_f + mic_nbat->nalloc*mic_nbat->fstride * 2;
		cpu_f = mic_f + mic_nbat->nalloc*mic_nbat->fstride * 3;
		mic_t_f = non_local_mic_t_f;
		cpu_t_f = non_local_cpu_t_f;
		if(non_local_mic_nalloc == 0){
			mic_nbat_x = nbvg->nbat->x;
			sig_mic_x = mic_nbat_x;
			#pragma offload target(mic:0)nocopy(mic_nbat),nocopy(mic_ic_tabq_coul_F),in(mic_nbat_x:length(mic_nbat->nalloc*mic_nbat->xstride)alloc_if(1) free_if(0)),in(mic_iconst:length(1)alloc_if(1) free_if(0))//signal(sig_mic_x)
			{
			...
			non_local_mic_nalloc = mic_nbat->nalloc;
		}
		else if(non_local_mic_nalloc != mic_nbat->nalloc){
			mic_nbat_x = nbvg->nbat->x;
			sig_mic_x = mic_nbat_x;
			// #pragma offload target(mic:0)nocopy(mic_nbat_x:length(non_local_mic_nalloc*mic_nbat->xstride)alloc_if(0) free_if(1))
			// {}
			// #pragma offload target(mic:1)nocopy(mic_nbat_x:length(non_local_mic_nalloc*mic_nbat->xstride)alloc_if(0) free_if(1))
			// {}
			// #pragma offload target(mic:2)nocopy(mic_nbat_x:length(non_local_mic_nalloc*mic_nbat->xstride)alloc_if(0) free_if(1))
			// {}
                         ...
                         int flags)
{
    ...
    if (bUseOrEmulGPU && !bDiffKernels)
    {
		...
		if (DOMAINDECOMP(cr)){
			...
			{
					mic_nbat = nbv->grp[eintNonlocal].nbat;	
					// printf("Nonlocal nbat:%d\n",mic_nbat->natoms);
					if(non_local_mic_nalloc == 0){
						#pragma offload target(mic:0)in(mic_nbat:length(1)alloc_if(1) free_if(0)),nocopy(mic_nbat_nbfp)
						{
							mic_nbat->nbfp = mic_nbat_nbfp;
						}
						#pragma offload target(mic:1)in(mic_nbat:length(1)alloc_if(1) free_if(0)),nocopy(mic_nbat_nbfp)
						{
							mic_nbat->nbfp = mic_nbat_nbfp;
						}
						#pragma offload target(mic:2)in(mic_nbat:length(1)alloc_if(1) free_if(0)),nocopy(mic_nbat_nbfp)
						{
							mic_nbat->nbfp = mic_nbat_nbfp;
						}
					}
				...
				
				{
					...
					if(non_local_mic_nalloc == 0){
						...
						non_local_mic_f = (real *)malloc(mic_nbat->nalloc*mic_nbat->fstride*4*sizeof(real));//mic_nbat->out[0].f;
						mic_f = non_local_mic_f;
						mic_f1 = mic_f + mic_nbat->nalloc*mic_nbat->fstride;
						mic_f2 = mic_f + mic_nbat->nalloc*mic_nbat->fstride * 2;
						cpu_f = mic_f + mic_nbat->nalloc*mic_nbat->fstride * 3;
						non_local_mic_t_f = (real *)malloc(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads*sizeof(real));
						mic_t_f = non_local_mic_t_f;
						non_local_cpu_t_f = (real *)malloc(mic_nbat->nalloc*mic_nbat->fstride*cpu_nthreads*sizeof(real));
						cpu_t_f = non_local_cpu_t_f;
						mic_nbat_type = mic_nbat->type;
						#pragma offload target(mic:0)nocopy(mic_nbat)\
													in(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(1) free_if(0))\
													nocopy(mic_t_f:length(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads)alloc_if(1) free_if(0))\
													nocopy(mic_f:length(mic_nbat->nalloc*mic_nbat->fstride)alloc_if(1) free_if(0))
						{
							mic_nbat->type = mic_nbat_type;
						}
						#pragma offload target(mic:1)nocopy(mic_nbat)\
													in(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(1) free_if(0))\
													nocopy(mic_t_f:length(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads)alloc_if(1) free_if(0))\
													nocopy(mic_f1:length(mic_nbat->nalloc*mic_nbat->fstride)alloc_if(1) free_if(0))
						{
							mic_nbat->type = mic_nbat_type;
						}
						#pragma offload target(mic:2)nocopy(mic_nbat)\
													in(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(1) free_if(0))\
													nocopy(mic_t_f:length(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads)alloc_if(1) free_if(0))\
													nocopy(mic_f2:length(mic_nbat->nalloc*mic_nbat->fstride)alloc_if(1) free_if(0))
						{
							mic_nbat->type = mic_nbat_type;
						}
						...
						// #pragma offload target(mic:0)nocopy(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(0) free_if(1))\
													// nocopy(mic_t_f:length(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads)alloc_if(0) free_if(1))\
													// nocopy(mic_f:length(mic_nbat->nalloc*mic_nbat->fstride)alloc_if(0) free_if(1))
													// {}
						// #pragma offload target(mic:1)nocopy(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(0) free_if(1))\
													// nocopy(mic_t_f:length(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads)alloc_if(0) free_if(1))\
													// nocopy(mic_f1:length(mic_nbat->nalloc*mic_nbat->fstride)alloc_if(0) free_if(1))
													// {}
						// #pragma offload target(mic:2)nocopy(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(0) free_if(1))\
													// nocopy(mic_t_f:length(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads)alloc_if(0) free_if(1))\
													// nocopy(mic_f2:length(mic_nbat->nalloc*mic_nbat->fstride)alloc_if(0) free_if(1))
													// {}	
						// free(non_local_mic_f);
						// free(non_local_mic_t_f);
						// free(non_local_cpu_t_f);
						...
						cpu_t_f = non_local_cpu_t_f;
						#pragma offload target(mic:0)nocopy(mic_nbat)\
													in(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(0) free_if(0))\
													nocopy(mic_t_f:length(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads)alloc_if(1) free_if(0))\
													nocopy(mic_f:length(mic_nbat->nalloc*mic_nbat->fstride)alloc_if(1) free_if(0))//signal(sig_mic_type)
						...
						#pragma offload target(mic:1)nocopy(mic_nbat)\
													in(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(0) free_if(0))\
													nocopy(mic_t_f:length(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads)alloc_if(1) free_if(0))\
													nocopy(mic_f1:length(mic_nbat->nalloc*mic_nbat->fstride)alloc_if(1) free_if(0))//signal(sig_mic_type)
						...
						#pragma offload target(mic:2)nocopy(mic_nbat)\
													in(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(0) free_if(0))\
													nocopy(mic_t_f:length(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads)alloc_if(1) free_if(0))\
													nocopy(mic_f2:length(mic_nbat->nalloc*mic_nbat->fstride)alloc_if(1) free_if(0))//signal(sig_mic_type)
			...
			{
				mic_shift_vec = mic_nbat->shift_vec;
				if(non_local_mic_nalloc == 0){
					#pragma offload target(mic:0)nocopy(mic_nbat),in(mic_shift_vec:length(SHIFTS)alloc_if(1) free_if(0))
					{
						mic_nbat->shift_vec = mic_shift_vec;
					}
					#pragma offload target(mic:1)nocopy(mic_nbat),in(mic_shift_vec:length(SHIFTS)alloc_if(1) free_if(0))
					{
						mic_nbat->shift_vec = mic_shift_vec;
					}
					#pragma offload target(mic:2)nocopy(mic_nbat),in(mic_shift_vec:length(SHIFTS)alloc_if(1) free_if(0))
					{
						mic_nbat->shift_vec = mic_shift_vec;
					}
				}
					...
					#pragma offload target(mic:0)nocopy(mic_nbat),in(mic_shift_vec:length(SHIFTS)alloc_if(0) free_if(0))
					{
						mic_nbat->shift_vec = mic_shift_vec;
					}
					#pragma offload target(mic:1)nocopy(mic_nbat),in(mic_shift_vec:length(SHIFTS)alloc_if(0) free_if(0))
					{
						mic_nbat->shift_vec = mic_shift_vec;
					}
					#pragma offload target(mic:2)nocopy(mic_nbat),in(mic_shift_vec:length(SHIFTS)alloc_if(0) free_if(0))
					{
						mic_nbat->shift_vec = mic_shift_vec;
					}
				}
        ...
        {
			{
				...
				mic_nbl = nbv->grp[eintNonlocal].nbl_lists.nbl[0];
				{
				#pragma offload target(mic:0)in(mic_nbl:length(1)alloc_if(1) free_if(0))
					{
					}
				#pragma offload target(mic:1)in(mic_nbl:length(1)alloc_if(1) free_if(0))
					{
					}
				#pragma offload target(mic:2)in(mic_nbl:length(1)alloc_if(1) free_if(0))
					{
					}
				}
				
				mic_nbl_sci = mic_nbl->sci;
				mic_nbl_cj4 = mic_nbl->cj4;
				mic_nbl_excl = mic_nbl->excl;
				
				#pragma offload target(mic:0)nocopy(mic_nbl)\
										in(mic_nbl_sci:length(mic_nbl->sci_nalloc)alloc_if(1) free_if(0))\
										in(mic_nbl_cj4:length(mic_nbl->cj4_nalloc)alloc_if(1) free_if(0))\
										in(mic_nbl_excl:length(mic_nbl->excl_nalloc)alloc_if(1) free_if(0))//signal(sig_mic_sci)
				{
					mic_nbl->sci = mic_nbl_sci;
					mic_nbl->cj4 = mic_nbl_cj4;
					mic_nbl->excl = mic_nbl_excl;
					
					 
				}
				#pragma offload target(mic:1)nocopy(mic_nbl)\
										in(mic_nbl_sci:length(mic_nbl->sci_nalloc)alloc_if(1) free_if(0))\
										in(mic_nbl_cj4:length(mic_nbl->cj4_nalloc)alloc_if(1) free_if(0))\
										in(mic_nbl_excl:length(mic_nbl->excl_nalloc)alloc_if(1) free_if(0))//signal(sig_mic_sci)
				{
				...
				#pragma offload target(mic:2)nocopy(mic_nbl)\
										in(mic_nbl_sci:length(mic_nbl->sci_nalloc)alloc_if(1) free_if(0))\
										in(mic_nbl_cj4:length(mic_nbl->cj4_nalloc)alloc_if(1) free_if(0))\
										in(mic_nbl_excl:length(mic_nbl->excl_nalloc)alloc_if(1) free_if(0))//signal(sig_mic_sci)
				{
    ...
    if (bUseOrEmulGPU)
    {
		...
		{
			mic_nbat = nbv->grp[eintLocal].nbat;
			// printf("local nbat:%d\n",mic_nbat->natoms);
			if(local_mic_nalloc == 0){
				#pragma offload target(mic:0)in(mic_nbat:length(1)alloc_if(1) free_if(0)),nocopy(mic_nbat_nbfp)
				{
		...
		
		{
			...
			if(local_mic_nalloc == 0){
				...
				local_mic_f = (real *)malloc(mic_nbat->nalloc*mic_nbat->fstride*4*sizeof(real));//mic_nbat->out[0].f;
				mic_f = local_mic_f;
				mic_f1 = mic_f + mic_nbat->nalloc*mic_nbat->fstride;
				mic_f2 = mic_f + mic_nbat->nalloc*mic_nbat->fstride * 2;
				cpu_f = mic_f + mic_nbat->nalloc*mic_nbat->fstride * 3;
				local_mic_t_f = (real *)malloc(mic_nbat->nalloc*mic_nbat->fstride*mic_nthreads*sizeof(real));
				mic_t_f = local_mic_t_f;
				local_cpu_t_f = (real *)malloc(mic_nbat->nalloc*mic_nbat->fstride*cpu_nthreads*sizeof(real));
				cpu_t_f = local_cpu_t_f;
				mic_nbat_type = mic_nbat->type;
				#pragma offload target(mic:0)nocopy(mic_nbat)\
											in(mic_nbat_type:length(mic_nbat->nalloc)alloc_if(1) free_if(0))\
		...
		
		{
			...
			mic_nbl = nbv->grp[eintLocal].nbl_lists.nbl[0];
			{
				#pragma offload target(mic:0)in(mic_nbl:length(1)alloc_if(1) free_if(0))
				{
				}
