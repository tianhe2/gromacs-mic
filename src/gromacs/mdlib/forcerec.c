                                 ...
                                 const gmx_gpu_opt_t *gpu_opt)
{
    ...
    *bUseGPU = FALSE;
    ...
    bEmulateGPUEnvVarSet = TRUE;//(getenv("GMX_EMULATE_GPU") != NULL);
                               ...
                               real                 rtab)
{
    ...
    else
    {
		...
		printf("in else ic:%x  tabq_size:%d\n",ic,ic->tabq_size);
    }
                       ...
                       real                        rtab)
{
    ...
    *interaction_const = ic;
    ...
    {
		printf("init_nbnxn_cuda_const here\n");
        nbnxn_cuda_init_const(fr->nbv->cu_nbv, ic, fr->nbv->grp);
		printf("init_nbnxn_cuda_const over\n");
	...
	
	{
		cpu_nthreads = 24;
		mic_iconst = ic;
		mic_nbat = fr->nbv->grp[0].nbat;
		mic_t_fshift = (real *)malloc(SHIFTS*DIM*mic_nthreads*sizeof(real));
		cpu_t_fshift = (real *)malloc(SHIFTS*DIM*cpu_nthreads*sizeof(real));
		mic_fshift = (real *)malloc(SHIFTS*DIM*sizeof(real));//mic_nbat->out[0].fshift;
		mic_Vc = (real *)malloc(4*sizeof(real));//mic_nbat->out[0].Vc;
		mic_Vvdw = (real *)malloc(4*sizeof(real));//mic_nbat->out[0].Vvdw;
		mic_fshift1 = (real *)malloc(SHIFTS*DIM*sizeof(real));//mic_fshift + SHIFTS*DIM;
		mic_Vc1 = mic_Vc + 1;
		mic_Vvdw1 = mic_Vvdw + 1;
		mic_fshift2 = (real *)malloc(SHIFTS*DIM*sizeof(real));//mic_fshift + SHIFTS*DIM*2;
		mic_Vc2 = mic_Vc + 2;
		mic_Vvdw2 = mic_Vvdw + 2;
		cpu_fshift = (real *)malloc(SHIFTS*DIM*sizeof(real));//mic_fshift + SHIFTS*DIM*3;
		cpu_Vc = mic_Vc + 3;
		cpu_Vvdw = mic_Vvdw + 3;
		mic_nbat_nbfp = mic_nbat->nbfp;
		mic_ic_tabq_coul_F = mic_iconst->tabq_coul_F;
		
		sig_mic_type = mic_nbat_nbfp;
		printf(" %x:mic_iconst->tabq_scale:%f\n",ic,mic_iconst->tabq_scale);
		#pragma offload target(mic:0)nocopy(mic_t_fshift:length(SHIFTS*DIM*mic_nthreads)alloc_if(1) free_if(0))\
								nocopy(mic_fshift:length(SHIFTS*DIM)alloc_if(1) free_if(0))\
								nocopy(mic_Vc:length(1)alloc_if(1) free_if(0))\
								nocopy(mic_Vvdw:length(1)alloc_if(1) free_if(0))\
								in(mic_nbat_nbfp:length(mic_nbat->ntype*mic_nbat->ntype*2)alloc_if(1) free_if(0))\
								// in(mic_iconst:length(1)alloc_if(1) free_if(0))\
		...
		#pragma offload target(mic:1)\
								in(mic_nbat_nbfp:length(mic_nbat->ntype*mic_nbat->ntype*2)alloc_if(1) free_if(0))\
								nocopy(mic_t_fshift:length(SHIFTS*DIM*mic_nthreads)alloc_if(1) free_if(0))\
								nocopy(mic_fshift1:length(SHIFTS*DIM)alloc_if(1) free_if(0))\
								nocopy(mic_Vc1:length(1)alloc_if(1) free_if(0))\
								nocopy(mic_Vvdw1:length(1)alloc_if(1) free_if(0))\
		...
		#pragma offload target(mic:2)\
								in(mic_nbat_nbfp:length(mic_nbat->ntype*mic_nbat->ntype*2)alloc_if(1) free_if(0))\
								nocopy(mic_t_fshift:length(SHIFTS*DIM*mic_nthreads)alloc_if(1) free_if(0))\
								nocopy(mic_fshift2:length(SHIFTS*DIM)alloc_if(1) free_if(0))\
								nocopy(mic_Vc2:length(1)alloc_if(1) free_if(0))\
								nocopy(mic_Vvdw2:length(1)alloc_if(1) free_if(0))\
    ...
    *nb_verlet = nbv;
    ...
    for (i = 0; i < nbv->ngrp; i++)
    {
        ...
        nbnxn_init_pairlist_set(&nbv->grp[i].nbl_lists,
                                ...
                                !nbnxn_kernel_pairlist_simple(nbv->grp[i].kernel_type),
                                nb_alloc, nb_free);
		printf("after init_pairlist_set\n");
        if (i == 0 ||
                   ...
                   real               print_force)
{
	...
	{
		mic_iconst = fr->ic;
		printf( "cpu mic_iconst :%x  mic_iconst->tabq_size:%d  mic_ic_tabq_coul_F[1]: %f\n",mic_iconst,mic_iconst->tabq_size,mic_ic_tabq_coul_F[1]);
		mic_ic_tabq_coul_F = mic_iconst->tabq_coul_F;
		#pragma offload target(mic:0)nocopy(mic_iconst),in(mic_ic_tabq_coul_F:length(mic_iconst->tabq_size)alloc_if(1) free_if(0))
		{	
		}
		#pragma offload target(mic:1)nocopy(mic_iconst),in(mic_ic_tabq_coul_F:length(mic_iconst->tabq_size)alloc_if(1) free_if(0))
		{
		}
		#pragma offload target(mic:2)nocopy(mic_iconst),in(mic_ic_tabq_coul_F:length(mic_iconst->tabq_size)alloc_if(1) free_if(0))
		{
		}
	}
}
